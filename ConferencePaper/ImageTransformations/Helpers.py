# source code from https://github.com/UjjwalSaxena/Automold--Road-Augmentation-Library
import cv2 as cv2
import glob
# import random
import math
...
def visualize(image_array, column=1, v_gap=0.1, h_gap=0.1, fig_size=(20,20), color_map=None, caption_array=-1):
    ...
    if(is_list(image_array)):
        ...
        #f,axes= plt.subplots(row, column, figsize=fig_size) # Jagan -- backward compatibility issue (Python 3 vs Python 2)
        f,axes = plt.subplots(int(row), int(column), figsize=fig_size)
...
#         plt.tight_layout(pad=0.4, w_pad=0.5, h_pad=1.0)
    #print(type(image_array))  #Jagan
    #print(is_numpy_array(image_array))  #Jagan
...
def load_images(path):
    ...
    for index in range(len(images)):
        ...
        #image_list.append(cv2.resize(image,(1280,720)))  # commented by Jagan
