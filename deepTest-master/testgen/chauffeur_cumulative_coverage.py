...
def chauffeur_guided(dataset_path):
    ...
    print("Now running coverage for seed image (baseline)")
    ...
    #         for i in input_images:
    #             j = i     # modified by Jagan
    #             chauffeur_queue.append(os.path.join(seed_inputs1, filelist1[j]))   # adding image to the queue
    #             # jagan changes starts
    #             image_file = cv2.imread(os.path.join(seed_inputs1, filelist1[j]))
    #             image = cv2.imread(image_file)
    #             covered, total, p = model.predict_fn(image)
    #             # jagan changes ends
    #     #exitcount = 0
