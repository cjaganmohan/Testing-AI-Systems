...
def chauffeur_testgen_coverage(index, dataset_path):
    ...
    seed_inputs1 = os.path.join(dataset_path, "Ch2_001/center/")
    seed_labels1 = os.path.join(dataset_path, "Ch2_001/CH2_final_evaluation.csv")
    ...
    with open('result/chauffeur_rq2_70000_images.csv', 'ab', 0) as csvfile:
        ...
        if index/2 == 0:
            if index%2 == 0:
                for j in xrange(20, 25):  # modified by Jagan
                    seed_image = cv2.imread(os.path.join(seed_inputs1, filelist1[j]))
                    model.predict_fn(seed_image, dummy=0)
            else:
                for j in xrange(450, 455):  # modified by Jagan
                    seed_image = cv2.imread(os.path.join(seed_inputs1, filelist1[j]))
                    model.predict_fn(seed_image, dummy=0)

            if index%2 == 0:
                input_images = xrange(21, 24)
            else:
                input_images = xrange(450, 455)
        ...
        if index/2 >= 1 and index/2 <= 10:
            ...
            if index%2 == 0:
                for j in xrange(20, 25):
                    seed_image = cv2.imread(os.path.join(seed_inputs1, filelist1[j]))
                    seed_image = image_translation(seed_image, params)
                    model.predict_fn(seed_image, dummy=0)
            else:
                for j in xrange(450, 455):
                    seed_image = cv2.imread(os.path.join(seed_inputs1, filelist1[j]))
                    seed_image = image_translation(seed_image, params)
                    model.predict_fn(seed_image, dummy=0)

            if index%2 == 0:
                input_images = xrange(20, 25)
            else:
                input_images = xrange(450, 455)
